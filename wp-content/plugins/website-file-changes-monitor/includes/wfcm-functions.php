	...
	if ( ! get_option( WFCM_OPT_PREFIX . 'version', false ) ) {
		foreach ( $wsal_plugins as $plugin ) {
			if ( is_plugin_active( $plugin ) ) {
				...
				if ( method_exists( 'WpSecurityAuditLog', 'get_instance' ) ) {
					$wsal = WpSecurityAuditLog::get_instance();
					// Set excluded post types in WSAL.
					$excluded_cpts = Settings_Helper::get_option_value( 'custom-post-types', array() );
				} else {
...
function wfcm_send_scan_fail_email( $send_mail = true, $error_type = 'generic' ) {
	...
	$body .= '<p>' . sprintf(
		...
		esc_html__( 'Please contact our support at %1$ssupport@melapress.com%2$s to help you with this issue.', 'website-file-changes-monitor' ),
