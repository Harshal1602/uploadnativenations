	...
	public static function output() {
		...
		$script_params = array(
				...
				'scanModal'      => array(
						...
						'emailSentLine2'                => __( 'If you have not received an email, please <a href="https://melapress.com/support/submit-ticket/" target="_blank">contact us</a> so we can help you troubleshoot the issue.', 'website-file-changes-monitor' ),
						...
						'actionsInfo'                   => array(
								...
								'readMoreLinkAllowed'  => '<a href="https://melapress.com/support/kb/website-file-changes-monitor-allowed-files-directories-root-core/?utm_source=plugins&utm_medium=link&utm_campaign=wfcm" target="_blank">' . __( 'Read more', 'website-file-changes-monitor' ) . '</a>',
								'readMoreLinkExcluded' => '<a href="https://melapress.com/support/kb/website-file-changes-monitor-excluding-files-directories-wordpress-file-changes-scan/?utm_source=plugins&utm_medium=link&utm_campaign=wfcm" target="_blank">' . __( 'Read more', 'website-file-changes-monitor' ) . '</a>',
						),
				...
				'scanErrorModal' => array(
						...
						'body'    => sprintf( __( 'Oops! Something went wrong with the scan. Please %s for assistance.', 'website-file-changes-monitor' ), '<a href="https://melapress.com/support/?utm_source=plugins&utm_medium=link&utm_campaign=wfcm" target="_blank">' . __( 'contact us', 'website-file-changes-monitor' ) . '</a>' ),
	...
	public static function add_messages() {
		...
		if ( ! empty( $admin_notices ) ) {
			...
			if ( isset( $admin_notices['previous-scan-fail-generic'] ) && ! empty( $admin_notices['previous-scan-fail-generic'] ) ) {
				$msg = '<p>' . sprintf(
						/* Translators: 1 - WP White Security support hyperlink. 2 - support link closer */
								__( 'We detected that a previous file integrity scan failed due to an unknown reason. Contact us at %1$ssupport@melapress.com%2$s to help you with this issue.', 'website-file-changes-monitor' ),
								'<a href="mailto:support@melapress.com" target="_blank">',
								'</a>'
						) . '</p>';
			...
			if ( isset( $admin_notices['previous-scan-fail-timeout'] ) && ! empty( $admin_notices['previous-scan-fail-timeout'] ) ) {
				...
				$msg = '<p>' . sprintf(
						/* Translators: 1 - WP White Security support hyperlink. 2 - support link closer */
								__( 'The last file integrity scan was halted %3$sbecause it took more than maximum scan time (3 minutes) to complete. Contact us at %1$ssupport@melapress.com%2$s to help you with this issue.', 'website-file-changes-monitor' ),
								'<a href="mailto:support@melapress.com" target="_blank">',
								'</a>',
								$date_and_time_of_failure
						) . '</p>';
				...
				if ( wfcm_get_setting( 'sha256-hashing', false ) ) {
					...
					$msg = '<p>' . sprintf(
							/* Translators: link fragments. */
									...
									'<a href="https://melapress.com/support/submit-ticket/" target="_blank">',
									'</a>'
							) . '</p>';
			...
			if ( isset( $admin_notices['files-limit'] ) && ! empty( $admin_notices['files-limit'] ) ) {
				...
				$msg = '<p>' . sprintf(
						/* Translators: %s: WP White Security support hyperlink. */
								...
								'<a href="mailto:support@melapress.com" target="_blank">' . __( 'our support', 'website-file-changes-monitor' ) . '</a>'
						) . '</p>';
			...
			if ( isset( $admin_notices['wp-repo-core-comms-failed'] ) && $admin_notices['wp-repo-core-comms-failed'] ) {
				$msg = '<p>' . sprintf(
						/* Translators: %s: WP White Security support hyperlink. */
								...
								'<a href="mailto:support@melapress.com" target="_blank">' . __( 'get in touch with us for assistance', 'website-file-changes-monitor' ) . '</a>'
						) . '</p>';
		...
		if ( $current_scan > 0 ) {
			$msg = '<p>' . sprintf(
				__( 'A file scan is currently running in the background. Currently, there are %s remaining areas to scan.', 'website-file-changes-monitor' ),
				$current_scan
			) . '</p>';
