...
class WFCM_REST_API {
    ...
    global $wpdb;
    $table_name = $wpdb->prefix . WFCM_FILE_EVENTS_TABLE;

		// Start to build the query.
    $events_to_delete = $wpdb->get_results( $wpdb->prepare( "SELECT event_id FROM %1s WHERE LOCATE( %s, %1s )", $table_name, $folder_path, $table_name . '.event_file_path' ) );
