...
class WFCM_Monitor {
	...
	public function check_changes_complete( $directory ) {
		...
		foreach ( $directories as $directory ) {
			...
			$options_to_update = $wpdb->get_results(
				$wpdb->prepare( "SELECT option_name FROM %1s WHERE option_name LIKE %s ESCAPE %s", $wpdb->options, $option_name, '#' ),
				ARRAY_A
			);
	...
	private function compute_differences_and_create_change_events( $filtered_stored_files, $filtered_scanned_files, $hash_comparator, $path_to_scan, $origin ) {
		...
		if ( in_array( 'added', $this->scan_settings['type'], true ) && count( $files_added ) > 0 ) {
			...
			foreach ( $files_added as $file => $file_hash ) {
				...
				if ( ! empty( $site_content->skip_files ) && ( in_array( $filename, $site_content->skip_files, true ) || in_array( $file, $this->scan_settings['exclude-files'], true ) ) ) {
		...
		if ( in_array( 'deleted', $this->scan_settings['type'], true ) && count( $files_removed ) > 0 ) {
			...
			foreach ( $files_removed as $file => $file_hash ) {
				...
				// Check if the filename is in excluded files list.
				if ( in_array( $filename, $this->scan_settings['exclude-files'], true ) || in_array( $file, $this->scan_settings['exclude-files'], true ) ) {
					continue; // If true, then skip the loop.
				}
	...
	public function check_directory_for_updates( $scanned_files, $directory ) {
		...
		if ( '/plugins' === $dirname ) {
			...
			// Set the count.
			if ( isset( $this->scan_changes_count['plugin_updates'] ) ) {
			...
			// Set the count.
			if ( isset( $this->scan_changes_count['theme_updates'] ) ) {
		...
		if ( in_array( 'added', $this->scan_settings['type'], true ) && count( $files_added ) > 0 && 'Plugin Update' !== $event_context ) {
	...
	public function check_for_changes( $path_to_scan ) {
		...
		if ( $run_wp_org_checksum_validation ) {
            ...
            // Only run if we have files to check.
            if ( $release_files_to_check ) {
	...
	public function get_current_number_of_active_bg_processes() {
		...
		$bg_jobs = $wpdb->get_results( $wpdb->prepare( "SELECT option_value FROM %1s WHERE option_name LIKE %s", $wpdb->options, '%_wfcm_scanner_%' ) );
	...
	public function cancel_bg_processes() {
		global $wpdb;
		$wpdb->query( $wpdb->prepare( 'DELETE FROM %1s WHERE option_name LIKE %s', $wpdb->options, '%_wfcm_scanner_%' ) );
