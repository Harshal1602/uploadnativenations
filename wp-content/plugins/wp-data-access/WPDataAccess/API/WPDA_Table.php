    ...
    public function lookup(
        ...
        if ( null === $wpdadb ) {
            ...
            if ( strpos( $column_value, ',' ) !== false ) {
                ...
                $dataset = $wpdadb->get_results( $wpdadb->prepare( $sql, array($column_key, implode( '`,`', $columns ), $tbl) ), 'OBJECT' );
            } else {
                $sql = "\n\t\t\t\t\t\tselect distinct `%1s` as 'key'\n\t\t\t\t\t\t, `%1s` as 'value' \n\t\t\t\t\t\tfrom `%1s` \n\t\t\t\t\t\t{$where}\n\t\t\t\t\t\torder by 2\n\t\t\t\t\t";
                $dataset = $wpdadb->get_results( $wpdadb->prepare( $sql, array($column_key, $column_value, $tbl) ), 'OBJECT' );
            }
